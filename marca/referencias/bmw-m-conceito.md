> **⚠️ Como usar este arquivo neste projeto:** referência puramente conceitual, instalada via `getdesign` pra inspirar o design da Prestige com a linguagem da BMW M. **Não usar** as cores, a tipografia (BMW Type Next Latin) nem os tokens de componente listados abaixo nos materiais da Prestige — isso continua vindo exclusivamente de `marca/design-guide.md`. Aproveitar só os **conceitos de composição**: fotografia full-bleed como protagonista, contraste de peso entre título e corpo de texto, ritmo espaçado entre seções, hairlines finas como divisor, hierarquia editorial "menos chrome, mais imagem", uso de uma cor de destaque só em momentos de marca.
